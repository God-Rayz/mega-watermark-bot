...
class MEGA:
    ...
    def rename_subfolders(self, mega_folder_path, multiple_models, watermark_text=FOLDER_NAME):
        ...
        for folder_path in subfolders:
            if folder_path == mega_folder_path:
                continue  

            folder_name = folder_path.split("/")[-1].lower()
            new_folder_name = "" # Initialize new_folder_name

            if folder_path.count("/") == mega_folder_path.count("/") + 1:
                if multiple_models:
                    new_folder_name = "Content" if folder_name_count.get("Content") is None else f"Content {folder_name_count.get('Content') + 1}"
                    folder_name_count["Content"] = folder_name_count.get("Content", 0) + 1
                    new_folder_name = f"{new_folder_name} {watermark_text}"
                else:
                    if any(kw.lower() in folder_name for kw in PICS_KEYWORDS):
                        new_folder_name = f"Pics {watermark_text}"
                    elif any(kw.lower() in folder_name for kw in VIDS_KEYWORDS):
                        new_folder_name = f"Vids {watermark_text}"
                    elif any(kw.lower() in folder_name for kw in PPV_KEYWORDS):
                        new_folder_name = f"PPV {watermark_text}"
                    elif any(kw.lower() in folder_name for kw in SITERIP_KEYWORDS):
                ...
                if new_folder_name in folder_name_count:
                    folder_name_count[new_folder_name] += 1
                    new_folder_name = f"{new_folder_name} {folder_name_count[new_folder_name]}"
                else:
                    folder_name_count[new_folder_name] = 1

            elif folder_path.count("/") == mega_folder_path.count("/") + 2:
                if multiple_models:
                    new_folder_name = f"{folder_name.capitalize()} {watermark_text}"
                else:
                    if any(kw in folder_name for kw in PICS_KEYWORDS):
                        new_folder_name = f"Pics {watermark_text}"
                    elif any(kw in folder_name for kw in VIDS_KEYWORDS):
                        new_folder_name = f"Vids {watermark_text}"
                    elif any(kw in folder_name for kw in PPV_KEYWORDS):
                        new_folder_name = f"PPV {watermark_text}"
                    elif any(kw in folder_name for kw in SITERIP_KEYWORDS):
                        new_folder_name = f"Siterip {watermark_text}"
                    else:
                        folder_content_type = self.check_folder_content(folder_path)
                ...
                if any(kw in folder_name for kw in PICS_KEYWORDS):
                    ...
                    new_folder_name = f"PPV {watermark_name}"
                elif any(kw in folder_name for kw in SITERIP_KEYWORDS):
                        new_folder_name = f"Siterip {watermark_text}"
                else:
                        folder_content_type = self.check_folder_content(folder_path)
            ...
            if new_folder_name:
                new_folder_path = "/".join(folder_path.split("/")[:-1]) + f"/{new_folder_name}"
                rename_command = f'mega-mv "{folder_path.lstrip("/")}" "{new_folder_path.lstrip("/")}"'
                rename_output, rename_returncode = self.run_command(rename_command)
