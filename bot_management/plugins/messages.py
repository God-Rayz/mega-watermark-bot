            ...
            if folder_size > 20.0:
                ...
                if additional_accounts_needed > 0:
                    await mega_link_confirm.edit_text(
                        ...
                        f"Please upload a text file with {additional_accounts_needed} additional email(s) (one per line) within 10 minutes.",
                        reply_markup=InlineMarkup.set_credentials_markup(),
                        disable_web_page_preview=True
                    )
                    
                    try:
                        # Wait for additional accounts file
                        accounts_answer = await app.listen.Message(app, id=str(message.chat.id), timeout=600)
                        if not accounts_answer.document:
                            await message.reply_text("No document received for additional accounts. Process cancelled.", reply_markup=InlineMarkup.set_credentials_markup())
                            return

                        accounts_file_name = accounts_answer.document.file_name
                        if not accounts_file_name.endswith('.txt'):
                            await message.reply_text("Invalid file type. Please upload a .txt file for additional accounts.")
                            return

                        # Download the additional accounts file
                        accounts_file_path = await accounts_answer.download(file_name="additional_accounts.txt")

                        # Read and validate additional emails
                        with open(accounts_file_path, 'r', encoding='utf-8') as file:
                            additional_emails = [line.strip() for line in file if line.strip()]
                        ...
                        if invalid_additional_emails:
                            alert_message = f"❌ {len(invalid_additional_emails)} invalid email(s) found in additional accounts file."
    ...
    if message.text.startswith("https://mega.nz/folder"):
        ...
        try:
            ...
            if answer.text:
                ...
                # For folders <= 20GB, proceed with single account processing
                await mega_link_confirm.edit_text(f"<i><b>MEGA Link:</b> {mega_link}</i>\n<i><b>Model Name:</b> {model_name}</i>\n<i><b>Size:</b> {folder_size:.2f} GB</i>\n\nPress <i>☑️ Confirm</i> to continue", reply_markup=InlineMarkup.confirm_process_markup(mega_link_id, model_name, multiple_models, skip_upload=0), disable_web_page_preview=True)
